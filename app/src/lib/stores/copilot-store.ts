...
export class CopilotStore {
   ...
   * @param model Optional model to use. If null/undefined, resolves via
   *   listModels: prefers {@link DefaultCopilotModel}, otherwise
  ...
  public async generateCommitMessage(
    ...
    const resolvedModel =
      model ?? getPreferredDefaultModel(await this.getCachedModels())
    ...
    try {
      ...
      session = await client.createSession({
        model: resolvedModel?.id ?? DefaultCopilotModel,
        reasoningEffort,
        systemMessage: {
