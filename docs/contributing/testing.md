...
| `BranchList` | `branch-list-test.tsx` | 5 | Default/recent/other grouping, selected branch propagation, branch-name filtering, filtered no-results state, create-branch and branch/filter callback mapping |
...
| `CommitConflictsWarning` | `commit-conflicts-warning-test.tsx`, `commit-conflicts-warning-actions-test.tsx` | 4 | Conflicted file list rendering, destructive action copy, Escape dismissal, destructive button submit wiring, commit-clear-reset flow |
| `CommitMessage` | `commit-message-test.tsx` | 3 | Summary-required submit state, summary/description commit context, placeholder prepopulation and co-author confirmation/trailers |
...
| `DiffOptions` | `diff-options-test.tsx` | 4 | Popover open/close state, whitespace toggle callback, unified/split display callbacks |
| `FilterChangesList` | `filter-changes-list-test.tsx` | 3 | File-list text filtering, filtered no-results clear-filters flow, hidden committed changes warning and filter adjustment wiring |
